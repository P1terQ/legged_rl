...
#include "legged_rl_controllers/AliengoController.h"

namespace legged {

void AliengoController::handleWalkMode() {
...
bool AliengoController::loadModel(ros::NodeHandle& nh) {
...
bool AliengoController::loadRLCfg(ros::NodeHandle& nh) {
...
void AliengoController::computeActions() {
...
void AliengoController::computeEncoder() {
  Ort::MemoryInfo memoryInfo = Ort::MemoryInfo::CreateCpu(OrtAllocatorType::OrtArenaAllocator, OrtMemType::OrtMemTypeDefault);
  std::vector<Ort::Value> inputValues;
...
void AliengoController::computeObservation() {
  const auto& info = leggedInterface_->getCentroidalModelInfo();
  ...
  vector_t gait(7);
  gait << 2.5, 0.5, 0.5, 0.0, 0.5, 0.08, 0.3;  // Aliengo
